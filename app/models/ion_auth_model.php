...
class Ion_auth_model extends CI_Model {
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $query = $this->db->select($this->identity_column . ', id, password, group_id,active')
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                ...
                $sqlsatker = " SELECT  coalesce(a.satuan_kerja_id,b.satuan_kerja_id) as satuan_kerja_id
  FROM spg_pegawai a left join m_spg_unit_kerja b on b.unit_kerja_id = a.unit_kerja_id where 
  peg_id = '" . $user->id . "' ";
                $querysatker = $this->db->query($sqlsatker)->row();
                ...
                $session_data = array(
                    ...
                    'satker_id' => $querysatker->satuan_kerja_id
                );
    ...
    public function user($id = NULL) {
       ...
       /* $this->limit(1);
        ...
        $this->users();*/
         $sql = ' SELECT karir.users.*, karir."users"."id" as id, karir."users"."id" as user_id FROM '
                 . 'karir."users" WHERE "karir"."users"."id" = '.$id;
    ...
    public function update_last_login($id) {
        ...
        $this->db->update($this->tables['users'], array('ip_address' => $_SERVER['REMOTE_ADDR'], 'last_login' => time()), array('id' => $id));
    ...
    public function get_user_by_id($id = false) {
        ...
        $sql = ' SELECT a.id, a.group_id, a.ip_address, a.username, a."password", a.salt, a.email,
       a.activation_code, a.forgotten_password_code, a.remember_code, a.created_on, 
       a.last_login, a.active,b."name",c.peg_nama,c.peg_nip,c.peg_nip_baru,b.name,
       d.jabatan_nama, e.unit_kerja_nama
  FROM karir.users a inner join karir.groups b on b.id = a.group_id
  inner join spg_pegawai c on c.peg_id = a.id 
  inner join m_spg_jabatan d on d.jabatan_id = c.jabatan_id 
  inner join m_spg_unit_kerja e on e.unit_kerja_id = c.unit_kerja_id WHERE a.id = ' . $id; 
        return $this->db->query($sql);
    }

    public function get_user_nameall(){
        if (empty($id)) {
            $id = $this->session->userdata('user_id');
        }

        $sql = ' SELECT a.peg_nama FROM spg_pegawai a WHERE a.peg_id != ' . $id;
        return $this->db->query($sql);
    }
